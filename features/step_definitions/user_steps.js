...
Given('I have provided my username as {string}', function (email, callback) {
  this.user.user_id = email
  callback()
})

Given('I have provided my password as {string}', function (password, callback) {
...
When('I logout', function () {
  ...
  return this.client.mutate({
                              mutation: gql `mutation logout($user_id: String!) {end_authentication(user_id: $user_id)}`,
                              variables: {
                                user_id: this.user.user_id
                              }
                            })
...
Then('I will be given a message that says {string}', function (message, callback) {
...
Then('I will be given the message {string}', function (message, callback) {
  expect(this.result.error).to.be.null
  expect(this.result.data).to.not.be.null
  expect(this.result.data.register.path).to.be.equal('user_id')
  expect(this.result.data.register.message).to.be.equal(message)
  callback()
